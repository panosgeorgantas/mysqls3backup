...
mysqldump $MYSQL_LOGIN $MYSQLDB | gzip -c > $BACKUP_DIR/$MYSQLDB/${MYSQLDB}_${TIMESTAMP}.sql.gz
...
for db in `echo 'show databases;' | mysql |grep -v information_schema | grep -v performance_schema | tail -n +2`;do
	...
	s3upload_file $BACKUP_DIR/${db}/${db}_${TIMESTAMP}.sql.gz `hostname -f`_`dmidecode -t 4 | grep ID | sed 's/.*ID://;s/ //g'`-${db}_${TIMESTAMP}.sql.gz || curl -s -X POST -H 'Content-type: application/json' --data "{\"text\":\"s3 upload of db backup of $db at `hostname -f` failed\"}" $SLACK_HOOK >/dev/null
